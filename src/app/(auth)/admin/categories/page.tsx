...
export default function AdminCategoriesPage() {
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(data.error || "Daftar kategori gagal dimuat.");
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.error || "Kategori gagal dihapus.");
      setItems((current) =>
        current.filter((item) => item.id !== deleteTarget.id),
      );
      setNotice({
        type: "success",
        text: `Kategori “${deleteTarget.name}” berhasil dihapus.`,
      });
  ...
  return (
    <AdminCrudPage>
      <AdminCrudHeader
        ...
        description="Kelola nama, slug URL, dan urutan kategori konten."
      ...
      {loading ? (
        <AdminCrudLoading label="Memuat daftar kategori..." />
      ) : loadError ? (
        <AdminCrudError message={loadError} onRetry={loadItems} />
      ) : (
        <AdminDataTable
          title="Daftar kategori"
          description={`${items.length} kategori tersimpan di CMS.`}
          items={items}
          columns={columns}
          getKey={(item) => item.id}
          searchFields={(item) => [item.name, item.slug]}
