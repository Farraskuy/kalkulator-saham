  ...
  useEffect(() => {
    ...
    fetch(`/api/articles?id=${encodeURIComponent(id)}`)
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(data.error || 'Artikel gagal dimuat.');
        return data.article;
      })
      .then((data) => {
        if (!active) return;
      ...
      .catch((fetchError) => {
        if (active) setError(fetchError instanceof Error ? fetchError.message : 'Artikel gagal dimuat.');
      });
    return () => {
      active = false;
    };
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-xl rounded-2xl border border-rose-200 bg-card p-6 text-center dark:border-rose-900">
        ...
        <h1 className="mt-3 text-lg font-bold text-main">Artikel tidak dapat dibuka</h1>
        <p className="mt-1 text-sm text-muted">{error}</p>
        <Link href="/admin/articles" className="mt-5 inline-flex items-center gap-2 rounded-xl bg-acc-blue px-4 py-2.5 text-sm font-semibold text-white">
