  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.error || 'Gagal membuat artikel baru.');
      }
    } catch {
      ...
      <div className="bg-card rounded-2xl border border-border-custom/50 shadow-sm p-6 sm:p-8 text-main space-y-6">
        <div>
          ...
          <p className="text-xs text-muted mt-1">
            Isi formulir di bawah ini untuk menambahkan artikel atau postingan blog baru ke dalam basis data.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder="Tuliskan isi artikel lengkap di sini menggunakan format Markdown..."
            />
          </div>
          ...
          <div className="pt-4 flex items-center justify-end gap-3 border-t border-border-custom/40">
            ...
            >
              <Save size={16} />
              <span>{submitting ? 'Menyimpan...' : 'Simpan Artikel Baru'}</span>
            </button>
          </div>
        </form>
      </div>
