...
export default function AdminSettingsPage() {
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        showToast(
          "Pengaturan Website, SEO & Disclaimer berhasil diperbarui!",
          "success",
        );
      } else {
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl">
      <AdminCrudHeader
        ...
        description="Kelola hal yang ditampilkan di website."
        icon={FileText}
      />
      ...
      <div className="bg-card rounded-3xl p-6 border border-border-custom max-w-3xl">
        <form onSubmit={handleSaveSettings} className="space-y-5">
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              <label className="text-[10px] font-bold text-muted block">
                Deskripsi Global Website (SEO Google / Mesin Pencari &amp;
                Footer)
              </label>
            </div>
            <textarea
            ...
            <p className="text-[10px] text-muted leading-relaxed pt-0.5">
              Teks deskripsi ini digunakan sebagai{" "}
              <strong>Meta Description SEO</strong> di Google / mesin pencari,
              kartu preview link media sosial (OpenGraph / Twitter), dan profil
              platform di bawah logo footer.
            </p>
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-muted block">
              Teks Syarat & Ketentuan (Disclaimer Footer)
            </label>
            <textarea
              className="w-full bg-page rounded-xl px-4 py-3 text-main font-semibold outline-none focus:border-acc-blue text-xs"
              rows={6}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-muted block">
              Teks Disclaimer Hasil Share Gambar (Watermark)
            </label>
            <textarea
              className="w-full bg-page rounded-xl px-4 py-3 text-main font-semibold outline-none focus:border-acc-blue text-xs"
              rows={4}
