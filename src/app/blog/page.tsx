...
export default async function PersonalBlogPage({
  ...
  const totalPages = Math.max(1, Math.ceil(listingArticles.length / BLOG_PAGE_SIZE));
        ...
        {articles.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 px-4 text-center space-y-8">
            <div className="space-y-1.5 max-w-md">
              <h2 className="text-2xl font-bold text-main">Belum Ada Artikel Dipublikasikan</h2>
          ...
          <>
            ...
            <section className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr] gap-[28px] mb-12">
              ...
              {mainHeadline && (
                <Link href={`/blog/${mainHeadline.slug}`} className="group bg-(--landing-card) rounded-2xl overflow-hidden no-underline text-inherit flex flex-col transition-all duration-200 ">
                  ...
                  <div className="p-[28px] flex flex-col gap-3 grow">
                    <span className="inline-block px-[10px] py-[4px] rounded-md text-[11px] font-bold uppercase tracking-[0.5px] bg-(--landing-soft-strong) text-(--landing-text) w-fit">{mainHeadline.category}</span>
                    <h2 className="m-0 text-[22px] font-bold text-(--landing-text) leading-[1.3] tracking-[-0.5px]">{mainHeadline.title}</h2>
                    <p className="m-0 text-xs sm:text-[13px] leading-[1.6] text-(--landing-muted)">{mainHeadline.excerpt}</p>
