...
export async function generateMetadata({
  ...
  return {
    ...
    description: `Hasil pencarian artikel dan jurnal trading saham BEI untuk kata kunci "${queryText}".`,
    robots: {
      index: false,
      follow: true,
    },
...
export default async function LandingTwoBlogSearchPage({
  searchParams,
}: {
  searchParams: Promise<{ q?: string; category?: string }>;
}) {
    ...
    <div className={styles.page}>
      ...
      <main className={styles.container} style={{ paddingTop: '32px' }}>
        ...
        <div className="flex items-center justify-center gap-2 overflow-x-auto pb-2 scrollbar-none mb-8">
          <Link
            href={q ? `/landing-2/blog/search?q=${encodeURIComponent(q)}` : '/landing-2/blog/search'}
          ...
          {categories.map((cat) => {
            ...
            const searchUrl = `/landing-2/blog/search?${q ? `q=${encodeURIComponent(q)}&` : ''}category=${encodeURIComponent(cat.name)}`;
            return (
              <Link
                key={cat.id}
                href={searchUrl}
        ...
        {/* SEARCH RESULTS SPLIT LAYOUT / UN-BOXED EMPTY STATE */}
        ...
        <div className={styles.footerContainer}>
          <div className={styles.footerGrid}>
            ...
            <div>
              ...
              <ul className={styles.footerNavList}>
                <li><Link href="/landing-2/blog">Jurnal Trading &amp; Catatan</Link></li>
          ...
          <div className={styles.footerBottom}>
            <div className={styles.footerCopyText}>
              &copy; {new Date().getFullYear()} HitungSaham.com • Catatan Trading &amp; Alat Analisis Saham BEI
            </div>
