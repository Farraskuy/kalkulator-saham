...
export async function getDynamicRoles(): Promise<DynamicRole[]> {
  try {
    ...
    if (setting?.value) {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        ...
        const filtered = parsed.filter(
          (r: DynamicRole) => r.id !== "ANALYST" && r.id !== "EDITOR",
        );
        const hasAdmin = filtered.some((r: DynamicRole) => r.id === "ADMIN");
        const hasContentManager = filtered.some(
          (r: DynamicRole) => r.id === "CONTENT_MANAGER",
        );
        ...
        if (hasContentManager) {
          result.push(
            filtered.find((r: DynamicRole) => r.id === "CONTENT_MANAGER")!,
          );
        } else {
    ...
    console.error(
      "Failed to load dynamic roles, falling back to defaults:",
      error,
    );
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!name || !name.trim()) {
      return NextResponse.json(
        { error: "Nama peran (role) wajib diisi." },
        { status: 400 },
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Role dengan nama atau ID tersebut sudah ada." },
        { status: 400 },
      );
    }
