...
export default function FaqEditorForm({
  ...
  const update = <K extends keyof FaqEditorData>(
    key: K,
    value: FaqEditorData[K],
  ...
  const insert = (before: string, after = "", fallback = "teks") => {
    ...
    const selected = form.answer.slice(start, end) || fallback;
    update(
      "answer",
  ...
  const save = async (event: React.FormEvent) => {
    ...
    if (!form.question.trim() || !form.answer.trim())
      return setError("Pertanyaan dan jawaban wajib diisi.");
    setSubmitting(true);
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : "FAQ gagal disimpan.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <form onSubmit={save} className="mx-auto max-w-5xl space-y-5 pb-10">
      <AdminEditorHeader
        ...
        description="Gunakan Markdown untuk jawaban yang terstruktur."
        actionLabel="Simpan FAQ"
      ...
      <section className="overflow-hidden rounded-2xl border border-border-custom bg-card">
        ...
        {tab === "write" ? (
          <textarea
            ...
            className="min-h-72 w-full resize-y bg-card px-5 py-4 font-mono text-sm leading-7 text-main outline-none"
