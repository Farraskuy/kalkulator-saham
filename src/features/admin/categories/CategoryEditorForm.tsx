...
export type CategoryEditorData = { id?: string; name: string; slug: string; order: number };
const slugify = (value: string) => value.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');

export default function CategoryEditorForm({ mode, initialData }: { mode: 'create' | 'edit'; initialData?: CategoryEditorData }) {
  const router = useRouter();
  const [form, setForm] = useState<CategoryEditorData>(initialData || { name: '', slug: '', order: 1 });
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : 'Kategori gagal disimpan.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <form onSubmit={save} className="mx-auto max-w-5xl space-y-5 pb-10">
      ...
      <section className="space-y-5 rounded-2xl border border-border-custom bg-card p-5 sm:p-6">
        <label className="block space-y-1.5">
          <span className="text-sm font-semibold text-main">Nama kategori *</span>
          <input
            autoFocus
            value={form.name}
            onChange={(event) => {
