...
export default function LandingBlogFilter({
  ...
  const categoryList = useMemo(() => {
    ...
    categories.forEach((category) => {
      if (category.name && !list.includes(category.name))
        list.push(category.name);
    });
    allArticles.forEach((article) => {
      if (article.category && !list.includes(article.category))
        list.push(article.category);
    });
    return list;
  }, [allArticles, categories]);

  const traderPicks = allArticles
    .filter((article) => article.isTraderPick)
    .slice(0, 3)
    .map((article) => ({
      title: article.title,
    ...
    <section id="articles" className="space-y-8">
      <div className="space-y-4">
        <div className="space-y-2">
          <h3 className="text-sm sm:text-base font-bold text-(--landing-text) m-0">
            Cari Artikel/Blog
          </h3>
          <BlogSearchDropdown
        ...
        <nav
          className="hidden sm:flex flex-wrap items-center gap-2 w-full"
          aria-label="Jelajahi kategori artikel"
        >
          <Link
            href="/blog/search"
            ...
            {articles.map((article) => (
              ...
              >
                ...
                <div className="flex min-w-0 flex-col justify-center">
                  ...
                  <h3 className="my-1 text-base font-bold text-(--landing-text) leading-[1.35]">
                    {article.title}
                  </h3>
        ...
        <aside className="flex flex-col gap-6">
          <div className="bg-(--landing-card) rounded-[14px] p-6 border border-(--landing-border)">
            <h3 className="text-[15px] font-bold text-(--landing-text) m-0 mb-4 pb-[10px] border-b border-(--landing-border)">
              Catatan Pilihan Teratas
            </h3>
            <div className="flex flex-col gap-3.5">
              {traderPicks.length === 0 && (
                <p className="text-xs leading-relaxed text-muted">
                  Belum ada artikel pilihan teratas yang ditandai.
                </p>
              )}
              {traderPicks.map((pick) => (
                ...
                >
                  <div className="text-[11px] text-(--landing-faint)">
                    <span className="font-bold text-sub">{pick.category}</span>{" "}
                    • {pick.readTime}
                  </div>
          ...
          <div className="bg-(--landing-inverse-bg) text-(--landing-inverse-text) border border-white/12 rounded-[14px] p-6 sm:p-7 flex flex-col gap-3">
            <div className="text-[11px] font-bold tracking-[1.5px] uppercase text-[#aebbd0]">
              <Calculator size={14} className="mr-1 inline" /> Simulasikan
              Portofolio
            </div>
            ...
            <p className="text-xs leading-[1.5] text-[#cbd5e1] m-0">
              Hitung persentase batas auto rejection dan kebutuhan lot pembelian
              tambahan saham kamu secara gratis.
            </p>
            <Link
              href="/#calculator"
