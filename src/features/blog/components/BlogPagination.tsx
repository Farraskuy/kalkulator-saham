...
export default function BlogPagination({ currentPage, totalPages, hrefForPage }: BlogPaginationProps) {
  ...
  return (
    <nav className="mt-8 flex flex-wrap items-center justify-center gap-2" aria-label="Navigasi halaman artikel">
      <Link
      ...
      {pages.map((page, index) => (
        <React.Fragment key={page}>
          {index > 0 && page - pages[index - 1] > 1 && <span className="px-1 text-sm text-muted">…</span>}
          <Link
            href={hrefForPage(page)}
            aria-current={page === currentPage ? 'page' : undefined}
            className={`grid h-10 min-w-10 place-items-center rounded-lg px-2 text-xs font-bold transition-colors ${
              page === currentPage ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-950' : 'text-main hover:bg-sub-slate'
            }`}
          >
            {page}
          </Link>
