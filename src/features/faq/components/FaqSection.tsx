      ...
      <section id="faq" className="space-y-4 scroll-m-50 w-full">
        {showHeader && (
          <div className="pb-2 text-center max-w-3xl mx-auto space-y-2">
            <div
              className={`text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 ${isAcme ? "text-main" : "text-acc-blue"}`}
            >
              <HelpCircle size={16} /> FAQ
            </div>
            <h2
              className={`text-2xl sm:text-3xl font-extrabold tracking-tight ${isAcme ? "text-main" : "text-main"}`}
            >
        ...
        <div
          className={`max-w-4xl mx-auto text-center p-8 py-40 rounded-2xl border e ? 'bg-card border-border-custom text-muted' : 'bg-card border-border-custom/40 text-muted'}`}
        >
          <h3
            className={`text-base font-bold mb-1 ${isAcme ? "text-main" : "text-main"}`}
          >
            Belum Ada Pertanyaan FAQ
          </h3>
          <p className="text-xs max-w-md mx-auto leading-relaxed">
            Saat ini belum ada pertanyaan FAQ yang dipublikasikan di database.
    ...
    <section id="faq" className="space-y-4 scroll-mt-20 w-full">
      {showHeader && (
        <div className="pb-2 text-center max-w-3xl mx-auto space-y-2">
          ...
          <p
            className={`text-xs sm:text-sm ${isAcme ? "text-muted" : "text-muted"}`}
          >
            Pertanyaan umum seputar fitur Hitungsaham.com, aturan BEI, serta
            strategi trading.
          </p>
        </div>
      )}
            ...
            >
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center items-start gap-1.5 sm:gap-3 leading-snug font-bold min-w-0 flex-1">
                  ...
                  <span className="text-xs sm:text-sm font-bold text-main break-words leading-snug">
                    {faq.question}
                  </span>
                </div>
                <div
                ...
                >
                  <ReactMarkdown
                    components={{
                      ...
                      strong: ({ children }) => (
                        <strong
                          className={`font-bold ${isAcme ? "text-main" : "text-main"}`}
                        >
                          {children}
                        </strong>
                      ),
